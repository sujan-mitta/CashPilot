...
const accounts = [
  { email: "mittal@company.com", name: "Aryan Mittal", initials: "AM", tone: "bg-brand-500/10 text-brand-300" },
  { email: "demo-guest@cashpilot.ai", name: "Demo Guest", initials: "DG", tone: "bg-ground-200 text-ink-300" },
];
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center p-4 font-sans">
      <motion.div
        ...
        className="bg-ground-100 border border-line-soft rounded-2xl w-full max-w-sm p-6 shadow-xl shadow-black/40 space-y-6"
      >
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-xl font-bold text-ink-100">Sign in with Google</h2>
          <p className="text-xs text-ink-300 font-semibold flex items-center justify-center gap-1.5">
            to continue to <PilotIcon className="w-3.5 h-3.5 text-brand-300 inline" /> <span className="font-extrabold text-ink-200">CashPilot</span>
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {selected ? (
              ...
              >
                ...
                <span className="text-xs text-ink-300 font-bold">Authenticating session...</span>
              </motion.div>
            ) : (
              <motion.div key="accounts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-2">
                {accounts.map((acc) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3.5 hover:bg-ground-200 border border-line-faint rounded-xl transition-colors duration-150 text-left outline-none"
                  >
                    <div>
                      <span className="text-xs font-bold text-ink-200 block">{acc.name}</span>
                      <span className="text-[10px] text-ink-400 font-semibold block">{acc.email}</span>
                    </div>
        ...
        <div className="text-[10px] text-ink-400 font-semibold leading-relaxed text-center">
