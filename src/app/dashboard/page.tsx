    ...
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        <Reveal variants={{ hidden: { opacity: 0, scale: 0.96 }, show: { opacity: 1, scale: 1, transition: { duration: 0.4, ease: EASE_OUT_EXPO } } }}>
          <Card tone="default" className="max-w-md border-risk-500/25 bg-risk-500/10 shadow-sm">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        <Reveal variants={{ hidden: { opacity: 0, scale: 0.96 }, show: { opacity: 1, scale: 1, transition: { duration: 0.4, ease: EASE_OUT_EXPO } } }}>
          <Card className="max-w-md shadow-sm">
            ...
            <h2 className="text-lg font-bold text-ink-100">Cash monitoring needs data</h2>
            <p className="text-ink-300 text-xs mt-2 leading-relaxed font-medium">
  ...
  return (
    <main className="flex-1 max-w-5xl mx-auto px-6 py-10 w-full space-y-8">
      <Reveal className="flex justify-between items-center">
        <h1 className="text-xl font-black text-ink-100 tracking-tight">Runway Forecast</h1>
      ...
      <Stagger className="space-y-8" stagger={0.08}>
        ...
        <StaggerItem>
          <Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 items-center">
              ...
              <div>
                <span className="text-[10px] text-ink-400 font-bold uppercase tracking-widest block mb-1.5">Status</span>
        ...
        {minProjected < 0 && (
          <StaggerItem>
            <Card className="border-risk-500/25 bg-risk-500/10">
              <div className="flex items-start gap-4">
                <AlertTriangle className="w-8 h-8 text-risk-400 flex-shrink-0 mt-1" />
                <div className="space-y-1 w-full">
                  <h3 className="text-sm font-black text-risk-400 uppercase tracking-tight">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold pt-2">
                    <div>
                      <span className="text-ink-300 font-medium block">Expected minimum balance:</span>
                      <span className="text-base font-black text-risk-400">{formatINR(minProjected)}</span>
                    </div>
                    <div>
                      <span className="text-ink-300 font-medium block">Required liquidity:</span>
                      <span className="text-base font-black text-ink-200">{formatINR(safetyThreshold)}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {edgeCase !== "CASE_A" && (
          <StaggerItem>
            <Card className="space-y-4">
              <h3 className="text-xs font-black text-ink-400 uppercase tracking-widest">Why Attention is Required</h3>

              <p className="text-sm font-semibold text-ink-200 leading-relaxed">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 pt-2 border-t border-line-faint">
                <div className="p-3 bg-ground-200 rounded-xl">
                  <span className="text-[10px] text-ink-400 font-bold block uppercase tracking-wider">Current Cash</span>
                  <span className="text-sm font-black text-ink-100 mt-1 block">{formatINR(currentCash)}</span>
                </div>
                <div className="p-3 bg-ground-200 rounded-xl">
                  <span className="text-[10px] text-ink-400 font-bold block uppercase tracking-wider">Expected Inflows</span>
                  <span className="text-sm font-black text-safe-400 mt-1 block">+{formatINR(totalInflows)}</span>
                </div>
                <div className="p-3 bg-ground-200 rounded-xl">
                  <span className="text-[10px] text-ink-400 font-bold block uppercase tracking-wider">Expected Outflows</span>
                  <span className="text-sm font-black text-risk-400 mt-1 block">-{formatINR(totalOutflows)}</span>
                </div>
                <div className="p-3 bg-ground-200 rounded-xl col-span-2 sm:col-span-1">
                  <span className="text-[10px] text-ink-400 font-bold block uppercase tracking-wider">Critical Obligations</span>
                  <span className="text-sm font-black text-ink-100 mt-1 block">
                    {formatINR(forecast.criticalObligations?.amount || 0)} ({forecast.criticalObligations?.count || 0} due)
                  </span>
                </div>
                <div className="p-3 bg-ground-200 rounded-xl col-span-2 sm:col-span-1">
                  <span className="text-[10px] text-ink-400 font-bold block uppercase tracking-wider">First Safety Breach</span>
                  <span className="text-sm font-black text-warn-400 mt-1 block">
          ...
          <StaggerItem>
            <Card className="space-y-4">
              <div className="flex justify-between items-center flex-wrap gap-4">
                <div>
                  <h3 className="text-xs font-black text-ink-400 uppercase tracking-widest">Required Operating Liquidity</h3>
                  <span className="text-2xl font-black text-ink-100 mt-1 block">{formatINR(safetyRequirement.requiredBuffer)}</span>
                </div>
              ...
              <details className="group border border-line-faint rounded-xl p-4 bg-ground-200/50">
                ...
                <div className="text-xs space-y-3 text-ink-300 pt-3 border-t border-line-faint font-semibold leading-relaxed group-open:block hidden">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div>
                      <span className="text-ink-400 block font-normal">Average Daily Outflow (Weighted):</span>
                      <span className="text-ink-100">{formatINR(safetyRequirement.averageDailyOutflow)}/day</span>
                    </div>
                    <div>
                      <span className="text-ink-400 block font-normal">Absolute Safety Floor:</span>
                    ...
                    <div>
                      <span className="text-ink-400 block font-normal">Historical Outflow Weighting:</span>
                      <span className="text-ink-100">70%</span>
                    </div>
                    <div>
                      <span className="text-ink-400 block font-normal">Projected Outflow Weighting:</span>
          ...
          <StaggerItem>
            <Card className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-ink-400 uppercase tracking-widest">Data Confidence</span>
                <Badge
                  tone={
              ...
              {safetyRequirement.dataWarnings && safetyRequirement.dataWarnings.length > 0 && (
                <div className="bg-warn-500/10 border border-warn-500/25 rounded-xl p-4 text-xs space-y-1.5">
                  <span className="font-extrabold text-warn-400 flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5" /> Data warnings detected:
                  </span>
                  <ul className="list-disc pl-4 space-y-1 text-warn-400 font-semibold">
        ...
        <StaggerItem>
          <Card>
            <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
              <div>
                <h2 className="text-md font-black text-ink-100 uppercase tracking-tight">
              ...
              <div className="flex items-center gap-4 text-xs font-bold text-ink-300">
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-0.5 border-t border-dashed border-risk-500/45 block" />
                  Deficit (₹0)
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-0.5 border-t border-dashed border-brand-500/45 block" />
          ...
          <Card>
            ...
            <div className="space-y-3.5 max-h-96 overflow-y-auto pr-2">
              {days.map((d: any, idx: number) => {
                ...
                return (
                  <div key={idx} className="flex flex-col gap-2 py-3 border-b border-line-faint last:border-0">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-extrabold text-ink-400">Day {idx + 1} ({new Date(d.date).toLocaleDateString("en-IN", { day: "numeric", month: "short" })})</span>
                      <div className="flex gap-4">
                        {d.expectedInflows > 0 && (
                          <span className="text-xs font-extrabold text-safe-400">+{formatINR(d.expectedInflows)}</span>
                        )}
                        {d.expectedOutflows > 0 && (
                          <span className="text-xs font-extrabold text-risk-400">-{formatINR(d.expectedOutflows)}</span>
                        )}
                      </div>
                    </div>
                    <div className="text-xs text-ink-300 font-semibold pl-4">
