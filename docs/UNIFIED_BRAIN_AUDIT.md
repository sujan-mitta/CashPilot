...
| **P11** Freshness ↔ stateVersion | ✅ done | this change | P7 delivered the core; this adds forecast-method identity + expiry (§32) |

### Regression floor after P11

| Check | Result |
|---|---|
| `npm run typecheck` | clean |
| `npm run lint` | 0 problems |
| `npm test` | 91 files, **1322 passed**, 5 skipped (982 baseline + 340 across P4–P13) |
...
## 18. Phase 11 — Forecast-method identity and expiry (spec §32)

P7 delivered the core of P11 (state ↔ freshness). This closes the two fields §32 lists that nothing carried: `forecastVersion` and `expiresAt`.

> **FILES:** `src/lib/engine/decisionValidity.ts` + tests (new); `prisma/schema.prisma`, `prisma/migrations/20260829000000_phase11_decision_validity/`, `financialConfig.ts`, `forecastEvent.ts`, `freshnessGate.ts`, `strategies/route.ts` (modified)
> **MIGRATION IMPACT:** two nullable columns on `Decision`, no defaults, **no backfill** — so no existing recommendation is retroactively expired.
> **TEST IMPACT:** +24. **SECURITY IMPACT:** none new.
> **ROLLBACK PLAN:** revert the `tightenForValidity` call in the gate; the columns then go unread.

### Two axes the existing checks structurally cannot see

| Axis | Why the fingerprint misses it |
|---|---|
| **METHOD** | Flipping `FORECAST_EVENT_PIPELINE` changes how numbers are produced while leaving every hashed *fact* identical. The fingerprint is byte-identical. |
| **AGE** | Nothing observed changed for a week. In a live ledger that is a reason to distrust the inputs, not reassurance about them (§55–56). |

### `forecastVersion` closes C-11 mechanically

C-11 said: when the pipeline is flipped, someone **must remember** to bump `SCORING_CONFIG_VERSION` in the same change, or strategies built the old way sail through the gate into a forecast that no longer matches them.

That is now automatic. Each decision records the pipeline identity that produced it (`ledger-v1` / `event-v1`); the gate compares it to the one in force. A test asserts the same decision passes before a flip and blocks after it. **The hazard is no longer a note in a document.**

### Deliberately not a second fingerprint

`forecastVersion` is *not* a hash of the forecast. `contextFingerprint` already hashes the facts the forecast consumed, and adding a second content hash would be exactly the two-sources-of-truth problem avoided throughout. This records the **method**, which is the part the fingerprint cannot represent.

I also chose not to add the pipeline identity *into* `contextFingerprint`: doing so would have re-hashed every decision on deploy and mass-stale-blocked every in-flight recommendation with an unattributable `UNKNOWN`, for no benefit while the flag is off. Recording it as a separate nullable column follows P7's pattern and is provably inert until something writes it.

### Same safety property as P7

`tightenForValidity` takes the more severe verdict, so it can block something that would have passed and never the reverse — asserted across all four freshness classifications. `UNTRACKED` returns the input **by identity**, so a pre-Phase-11 decision is untouched.

### The one real behaviour change

Unlike P7, this phase **does** populate its columns: `strategies/route.ts` records both on every new decision. `forecastVersion` is a pure code identity, so recording it is free. `expiresAt` is a genuine new blocking condition — a recommendation older than `DECISION_TTL_HOURS` (168h) is refused even when nothing changed.

Seven days against a 14-day horizon: a plan built to protect a fortnight has spent half of it before expiring. In any active ledger, content or state changes block sooner; the TTL only catches a week of total silence.

### Limitations

1. **The TTL is reasoned, not calibrated** — as with C-12.
2. **Existing decisions never expire.** No backfill, by design; they stay on the old behaviour until regenerated.
3. **Expiry is not surfaced in the UI** — a user sees it only when execution is refused.
4. **`strategyVersion` from §32 is still not modelled.** `Strategy` rows are immutable and replaced rather than versioned, so a version counter would have no distinct meaning today.
